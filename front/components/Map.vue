<template>
  <section class="mapsection" name="image-map" v-if="coworkings.length > 0" style="height: 400px;">
    ...
    >
      ...
      <l-popup v-if="userLocationMarker.latLng">
        Your location
        <p>{{ userLocationMarker.latLng[0] }}, {{ userLocationMarker.latLng[1] }}</p>
      </l-popup>
      </l-marker>
      <l-marker
...
<script setup>
import { LMap, LTileLayer, LControlLayers, LMarker, LPopup, LIcon } from '@vue-leaflet/vue-leaflet';
...
const osmAttrib = ref('&copy; <a href="http://www.openstreetmap.org">OpenStreetMap</a> contributors');
...
const markerData = computed(() => {
  return props.coworkings.map(coworking => {
    ...
    const photoURL = coworking.coworking_photo ? `${baseURL}/${coworking.coworking_photo}` : './default-coworking.png';
    const popupContent = `
...
function getLocation() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      userLocationMarker.latLng = [position.coords.latitude, position.coords.longitude];
